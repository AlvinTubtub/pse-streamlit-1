...
def main():
    ...
    # Read the ?page= param the dashboard's nav() function sets when the user
    # enters/leaves the Live Prediction tab, so the uploader only shows there
    # and the dashboard reopens on the tab the user was actually on.
    active_page = st.query_params.get("page", "home")
    if active_page not in ("home", "live"):
        active_page = "home"
